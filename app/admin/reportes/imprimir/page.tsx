...
export default async function PrintableReport({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  const events = await db.paymentEvent.findMany({ where: { createdAt: { gte: getBusinessDateRange(from).start, lt: getBusinessDateRange(to).end } } });
  ...
  return <main className="print-report"><header><div><p>Reporte de ventas</p><h1>{restaurant.name}</h1><span>{from} — {to}</span></div><PrintButton /></header><section className="print-report__summary"><div><span>Ingreso bruto</span><strong>{formatPrice(income)}</strong></div><div><span>Reembolsos</span><strong>{formatPrice(refunds)}</strong></div><div><span>Ingreso neto</span><strong>{formatPrice(income - refunds)}</strong></div><div><span>Pedidos</span><strong>{orders.length}</strong></div></section><p>Ingresos por fecha de cobro (Ecuador). El listado corresponde a la fecha de creación de los pedidos.</p><table><thead><tr><th>Fecha</th><th>Pedido</th><th>Canal</th><th>Estado</th><th>Método</th><th>Total</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id}><td>{order.businessDate}</td><td>#{order.dailyNumber}</td><td>{order.mode}</td><td>{order.status}</td><td>{order.paymentMethod ?? "—"}</td><td>{formatPrice(order.totalCents)}</td></tr>)}</tbody></table><footer>Generado por {session.name} · {new Date().toLocaleString("es-EC", { timeZone: "America/Guayaquil" })}</footer></main>;
}
