...
import { Check, ClipboardCheck, Phone, Printer, RefreshCw, Truck } from "lucide-react";
...
import { DeliveryIntervention } from "./delivery-intervention";
import { formatPrice } from "@/lib/format";
import { paymentMethodLabels } from "@/lib/payment-labels";
import { requestJson, SessionExpiredError } from "./admin-api";

type DeliveryFeed = { orders: DeliveryOrderView[]; drivers: { id: string; name: string }[]; canOverride?: boolean; canCollectCash: boolean; allowedPaymentMethods: PaymentMethod[]; total: number; page: number; pageSize: number };
...
export function DeliveryBoard({ manager }: { manager: boolean }) {
  ...
  async function act(order: DeliveryOrderView, action: "ASSIGN" | "DISPATCH" | "DELIVER" | "PAYMENT" | "REPORT_ISSUE" | "RETRY", driverId?: string, overrideReason?: string) {
    if (busy.current) return;
    const reason = ["REPORT_ISSUE", "RETRY"].includes(action) ? overrideReason ?? window.prompt(action === "REPORT_ISSUE" ? "¿Qué impidió la entrega? Indica el motivo (mínimo 4 caracteres)." : "¿Cómo se resolvió la incidencia? Indica el motivo del reintento.")?.trim() : undefined;
    ...
    try {
      ...
      else await requestJson(`/api/admin/deliveries/${order.id}`, "PATCH", { action, version: order.version, ...(reason ? { reason } : {}), ...(driverId ? { driverId } : {}), ...(overrideReason ? { override: true, overrideReason } : {}) });
      setPaymentId(null);
  ...
  return <section className="delivery-board">
    ...
    <div className="delivery-board__list">{loaded && feed.orders.map((order) => {
      ...
      return <article className="delivery-ticket" key={order.id}>
        ...
        <footer>
          {!manager && view === "active" && order.deliveryStatus === "OUT_FOR_DELIVERY" && <button type="button" className="button button--line" disabled={pending !== null} onClick={() => void act(order, "REPORT_ISSUE")}>No pude entregar</button>}
          ...
          <a className="delivery-ticket__receipt" aria-label="Ver / imprimir comprobante" href={`/admin/pedidos/${order.id}/comprobante`} target="_blank" rel="noopener noreferrer"><Printer aria-hidden="true" />Comprobante</a>
          {!manager && view === "active" && order.assignedDriver && order.status === "READY" && order.deliveryStatus === "PENDING" && <button type="button" className="button button--solid" disabled={pending !== null} onClick={() => void act(order, "DISPATCH")}><Truck aria-hidden="true" />Salir a reparto</button>}
          {!manager && view === "active" && order.status === "READY" && order.deliveryStatus === "OUT_FOR_DELIVERY" && <button type="button" className="button button--solid" disabled={pending !== null} onClick={() => void act(order, "DELIVER")}><Check aria-hidden="true" />Confirmar entrega</button>}
          {manager && view === "active" && feed.canOverride && <DeliveryIntervention key={`${order.id}-${order.version}`} order={order} pending={pending !== null} onConfirm={(action, reason) => act(order, action, undefined, reason)} />}
    ...
    {view === "history" && loaded && (feed.total > 20 || page > 1) && <nav className="delivery-pagination" aria-label="Páginas de entregas"><button type="button" className="button button--line" disabled={page <= 1 || pending !== null} onClick={() => { setPage(page - 1); setLoaded(false); setPaymentId(null); }}>Anterior</button><span>{page} / {Math.max(1, Math.ceil(feed.total / 20))}</span><button type="button" className="button button--line" disabled={page * 20 >= feed.total || pending !== null} onClick={() => { setPage(page + 1); setLoaded(false); setPaymentId(null); }}>Siguiente</button></nav>}
  </section>;
}
