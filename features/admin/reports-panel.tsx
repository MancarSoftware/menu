...
type Receipt = { order: OrderView; paymentEvents: { id: string; type: string; method: string; amountCents: number; reason: string; actorName: string; createdAt: string }[]; statusHistory: { id: string; status: string; actor: string; createdAt: string }[] };
const today = getBusinessDate();
function daysAgo(days: number) { const date = new Date(`${today}T12:00:00-05:00`); date.setDate(date.getDate() - days); return date.toISOString().slice(0, 10); }
function weekStart() { const date = new Date(`${today}T12:00:00-05:00`); const day = date.getDay() || 7; date.setDate(date.getDate() - day + 1); return date.toISOString().slice(0, 10); }
function monthStart() { return `${today.slice(0, 8)}01`; }

export function ReportsPanel() {
  const [filters, setFilters] = useState({ from: today, to: today, status: "", paymentMethod: "", mode: "", table: "" });
  ...
  async function openReceipt(order: OrderView) {
    try { setReceipt(await requestJson<Receipt>(`/api/admin/orders/${order.id}/receipt`)); }
  ...
  return <div className="reports-panel">
    ...
    {receipt && <div className="admin-modal" role="dialog" aria-modal="true"><article className="admin-modal__panel receipt-modal"><button className="admin-modal__close" onClick={() => setReceipt(null)} aria-label="Cerrar"><X /></button><p className="eyebrow">{receipt.order.businessDate}</p><h2>Comprobante #{receipt.order.orderNumber}</h2><p>{receipt.order.mode === "DINE_IN" ? `Mesa ${receipt.order.table?.number}` : receipt.order.mode === "DELIVERY" ? "Delivery" : "Retiro"} · {receipt.order.status}</p>{receipt.order.customerName && <p>{receipt.order.customerName} · {receipt.order.customerPhone}</p>}<div className="receipt-lines">{receipt.order.items.map((item) => <div key={item.id}><span>{item.quantity} × {item.productName}<small>{item.customization.join(" · ")}</small></span><strong>{formatPrice(item.lineTotalCents)}</strong></div>)}</div><div className="order-receipt__total"><span>Total</span><strong>{formatPrice(receipt.order.totalCents)}</strong></div><h3>Pagos y correcciones</h3>{receipt.paymentEvents.map((event) => <p key={event.id}>{event.type} · {event.method} · {formatPrice(event.amountCents)} · {event.actorName}</p>)}<div className="report-actions">{receipt.paymentEvents.some((event) => event.type === "PAYMENT") && <button className="button button--line" onClick={refundPayment}>Reembolsar</button>}{receipt.paymentEvents.some((event) => event.type === "PAYMENT") && <button className="button button--line" onClick={changePaymentMethod}>Corregir método</button>}<button className="button button--solid" onClick={() => window.print()}><Printer />Imprimir comprobante</button></div></article></div>}
  </div>;
}
