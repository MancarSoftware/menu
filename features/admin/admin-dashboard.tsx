  ...
  return (
    <div className="admin-shell">
      <aside className="admin-sidebar">
        <div><p className="eyebrow">Brasa Norte</p><strong>Panel de carta</strong></div>
        <nav aria-label="Secciones de administración">
          ...
          <button data-active={tab === "items"} onClick={() => setTab("items")}><UtensilsCrossed aria-hidden="true" />Platos</button>
      ...
      <section className="admin-workspace">
        <header className="admin-topbar"><div><p className="eyebrow">Operación / {new Date().toLocaleDateString("es-EC")}</p><h1>{tab === "overview" ? "Resumen" : tab === "categories" ? "Categorías" : tab === "items" ? "Platos" : "La casa"}</h1></div><a className="button button--line" href="/menu" target="_blank">Ver carta <ExternalLink aria-hidden="true" /></a></header>
...
function Overview({ categories, itemCount, availableCount, onNavigate }: { categories: MenuCategoryView[]; itemCount: number; availableCount: number; onNavigate: (tab: Tab) => void }) {
  return (
    <div className="admin-overview">
      <section className="admin-kpis"><div><span>Categorías activas</span><strong>{categories.filter((item) => item.isActive).length}<small> / {categories.length}</small></strong></div><div><span>Platos disponibles</span><strong>{availableCount}<small> / {itemCount}</small></strong></div><div><span>Destacados</span><strong>{categories.flatMap((c) => c.items).filter((i) => i.isFeatured).length}</strong></div></section>
      <section className="admin-overview__lead"><p className="eyebrow">Estado de servicio</p><h2>{availableCount === itemCount ? "La carta está completa." : `${itemCount - availableCount} platos fuera de servicio.`}</h2><p>Los cambios publicados se reflejan en la carta pública al actualizar la página.</p><button className="button button--solid" onClick={() => onNavigate("items")}>Gestionar platos</button></section>
...
function emptyItem(categories: MenuCategoryView[]): ItemForm { return { name: "", slug: "", shortDescription: "", description: "", price: "", imageUrl: "/images/hero-pulpo.webp", categoryId: categories[0]?.id ?? "", isAvailable: true, isFeatured: false, isChefRecommendation: false, displayOrder: 0, dietaryTags: "", ingredients: "", allergens: "", spicyLevel: "" }; }
...
function ItemManager({ categories, run }: { categories: MenuCategoryView[]; run: (action: () => Promise<void>, success: string) => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setPending(true); try { await run(() => requestJson(editingId ? `/api/items/${editingId}` : "/api/items", editingId ? "PATCH" : "POST", payload()).then(() => undefined), editingId ? "Plato actualizado y publicado." : "Plato creado y publicado."); reset(); } finally { setPending(false); } }
  async function remove(item: MenuItemView) { if (!window.confirm(`¿Eliminar “${item.name}”? Esta acción no se puede deshacer.`)) return; await run(() => requestJson(`/api/items/${item.id}`, "DELETE").then(() => undefined), "Plato eliminado."); if (editingId === item.id) reset(); }
  async function toggle(item: MenuItemView) { await run(() => requestJson(`/api/items/${item.id}`, "PATCH", { ...item, categoryName: undefined, categorySlug: undefined, priceCents: item.priceCents, dietaryTags: item.dietaryTags, ingredients: item.ingredients, allergens: item.allergens, isAvailable: !item.isAvailable }).then(() => undefined), item.isAvailable ? "Plato marcado como no disponible." : "Plato disponible nuevamente."); }
  async function move(item: MenuItemView, direction: -1 | 1) { const group = categories.find((category) => category.id === item.categoryId)?.items ?? []; const index = group.findIndex((candidate) => candidate.id === item.id); const next = index + direction; if (next < 0 || next >= group.length) return; const ids = group.map((candidate) => candidate.id); [ids[index], ids[next]] = [ids[next], ids[index]]; await run(() => requestJson("/api/items/reorder", "PATCH", { ids }).then(() => undefined), "Orden de platos actualizado."); }
  ...
  return (
    <div className="admin-items-layout">
      <form className="admin-editor admin-item-editor" onSubmit={submit}><header><p className="eyebrow">{editingId ? "Editar plato" : "Alta de plato"}</p><h2>{editingId ? form.name : "Nuevo plato"}</h2></header><div className="admin-image-field"><div>{form.imageUrl && <Image src={form.imageUrl} alt="Vista previa" fill sizes="300px" />}</div><label className="button button--line"><ImagePlus aria-hidden="true" />{uploading ? "Subiendo…" : "Cambiar imagen"}<input type="file" accept="image/png,image/jpeg,image/webp" disabled={uploading} onChange={(e) => { const file = e.target.files?.[0]; if (file) upload(file).catch((error) => window.alert(error.message)); }} /></label></div><div className="form-grid"><div className="form-field"><label htmlFor="item-name">Nombre</label><input id="item-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value, slug: editingId ? form.slug : slugify(e.target.value) })} required /></div><div className="form-field"><label htmlFor="item-price">Precio USD</label><input id="item-price" type="number" step="0.01" min="0" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} required /></div></div><div className="form-grid"><div className="form-field"><label htmlFor="item-slug">Identificador URL</label><input id="item-slug" value={form.slug} onChange={(e) => setForm({ ...form, slug: slugify(e.target.value) })} required /></div><div className="form-field"><label htmlFor="item-category">Categoría</label><select id="item-category" value={form.categoryId} onChange={(e) => setForm({ ...form, categoryId: e.target.value })} required>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></div></div><div className="form-field"><label htmlFor="item-short">Descripción breve</label><input id="item-short" value={form.shortDescription} onChange={(e) => setForm({ ...form, shortDescription: e.target.value })} required maxLength={180} /></div><div className="form-field"><label htmlFor="item-description">Descripción completa</label><textarea id="item-description" rows={4} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required /></div><div className="form-field"><label htmlFor="item-ingredients">Ingredientes <small>separados por coma</small></label><input id="item-ingredients" value={form.ingredients} onChange={(e) => setForm({ ...form, ingredients: e.target.value })} /></div><div className="form-grid"><div className="form-field"><label htmlFor="item-tags">Etiquetas dietarias</label><input id="item-tags" value={form.dietaryTags} onChange={(e) => setForm({ ...form, dietaryTags: e.target.value })} placeholder="Vegano, Sin gluten" /></div><div className="form-field"><label htmlFor="item-allergens">Alérgenos</label><input id="item-allergens" value={form.allergens} onChange={(e) => setForm({ ...form, allergens: e.target.value })} placeholder="Lácteos, Frutos secos" /></div></div><div className="form-grid form-grid--checks"><label className="check-field"><input type="checkbox" checked={form.isAvailable} onChange={(e) => setForm({ ...form, isAvailable: e.target.checked })} /><span>Disponible</span></label><label className="check-field"><input type="checkbox" checked={form.isFeatured} onChange={(e) => setForm({ ...form, isFeatured: e.target.checked })} /><span>Destacado</span></label><label className="check-field"><input type="checkbox" checked={form.isChefRecommendation} onChange={(e) => setForm({ ...form, isChefRecommendation: e.target.checked })} /><span>Selección del chef</span></label><div className="form-field"><label htmlFor="item-spicy">Picante 0–3</label><select id="item-spicy" value={form.spicyLevel} onChange={(e) => setForm({ ...form, spicyLevel: e.target.value })}><option value="">No aplica</option><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select></div></div><div className="admin-editor__actions"><button type="button" className="button button--line" onClick={reset}>Limpiar</button><button type="submit" className="button button--solid" disabled={pending || uploading}>{pending ? "Publicando…" : editingId ? "Guardar plato" : "Publicar plato"}</button></div></form>
