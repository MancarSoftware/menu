...
export function CashRegister() {
  ...
  const load = useCallback(async () => { const request = ++latestRequest.current; try { const result = await requestJson<{ shifts: CashShiftView[] }>("/api/admin/cash-shifts"); if (request === latestRequest.current) setShifts(result.shifts); } catch (error) { if (request === latestRequest.current) setMessage(error instanceof Error ? error.message : "No pudimos cargar la caja."); } }, []);
  useLiveRefresh(load);
