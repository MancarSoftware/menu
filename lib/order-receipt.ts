...
export async function getOrderReceipt(id: number, actor: { id: string; role: StaffRole }) {
  ...
  return {
    ...
    paymentEvents: order.paymentEvents.map((event) => {
      return { id: event.id, type: event.type, method: event.method, amountCents: event.amountCents, reason: event.reason, actorName: actorNames.get(event.id)!, createdAt: event.createdAt.toISOString() };
    }),
  };
}
