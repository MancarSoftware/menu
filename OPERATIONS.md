...
Order creation increments the daily counter with an atomic PostgreSQL upsert. One parameterized SQL statement inserts the order, items and initial history and updates the table. PostgreSQL's implicit statement transaction commits or rolls back all of these changes together; do not wrap it in a Prisma interactive transaction, whose connection acquisition and extra round trips caused transaction-start timeouts during a 120-order burst. The unique request ID still prevents duplicates; new public/order-item/history IDs are opaque UUID strings, while existing IDs remain valid. Response relation reads run after commit. Deadlocks/write conflicts receive at most four attempts with jitter; exhausted retries return a recoverable 503. Pool exhaustion (`P2024`), including response reads after a successful commit, returns 503 with `Retry-After: 3` instead of a generic 500. Retrying must preserve `clientRequestId` to recover an already committed order without duplication. No migration, new dependency or connection-limit change is required.
