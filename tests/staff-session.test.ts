...
beforeEach(() => {
  ...
  mocks.findUnique.mockResolvedValue({ id: "staff1", name: "Test Driver", email: "driver@example.invalid", role: "DRIVER", isActive: true, canCollectCash: false, canCollectCard: false, canCollectTransfer: false, passwordChangedAt: new Date(1000000) });
});

it("uses current database permissions rather than stale JWT role claims", async () => {
  expect(await getSession()).toEqual({ id: "staff1", name: "Test Driver", email: "driver@example.invalid", role: "DRIVER", canCollectCash: false, canCollectCard: false, canCollectTransfer: false });
});
